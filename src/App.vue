<template>
  ...
  <div class="update-process-container" v-show="showUpdateProcessDiv">
    ...
    <el-progress
      :text-inside="true"
      :stroke-width="15"
      :percentage="versionDownPercentage"
      :striped="true"
      :striped-flow="true"
    />
  </div>
...
export default {
  ...
  methods: {
      ...
      this.$nextTick(() => {
        ...
        if (done) {
          if (progress === 100) {
            ...
            this.downloadFinishedList.splice(0, 0, {
              id,
              path,
              resolution,
              size,
          ...
          localStorage.setItem(
            "downloadList",
            JSON.stringify(this.downloadList)
          );
          localStorage.setItem(
            "downloadFinishedList",
            JSON.stringify(this.downloadFinishedList)
          );
        } else {
          if (index > -1) {
    ...
    loadDownloadFinishedList() {
      let downloadFinishedListStr = getLocalStorage(
        "downloadFinishedList",
        "[]",
        "String"
      );
      if (downloadFinishedListStr != null) {
        ...
        if (this.downloadFinishedList.length > 20) {
          this.downloadFinishedList.splice(
            19,
            this.downloadFinishedList.length - 20
          );
          localStorage.setItem(
            "downloadFinishedList",
            JSON.stringify(this.downloadFinishedList)
    ...
    startApp() {
      let localStorageMap = {
        apiParams: getLocalStorage(
          "apiParams",
          "categories=111&purity=100&sorting=hot&order=desc",
          "String"
        ),
